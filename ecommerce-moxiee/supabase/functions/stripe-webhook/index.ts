...
import { getCurrency } from "../_shared/currency.ts";
  ...
  try {
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        // Fetch the order's fx_rate/currency (set at checkout time) so we
        // can convert Stripe's charged amount back into USD — orders.total/
        // tax always stay in USD, our bookkeeping base currency, no matter
        // what currency the customer actually paid in.
        const { data: orderForFx } = await supabaseAdmin
          .from("orders")
          .select("currency, fx_rate")
          .eq("id", orderId)
          .single();
        const currency = getCurrency(orderForFx?.currency);
        const fxRate = orderForFx?.fx_rate ?? currency.rate ?? 1;
        const divisor = currency.zeroDecimal ? 1 : 100;
        const totalInChargedCurrency = (session.amount_total ?? 0) / divisor;
        const taxInChargedCurrency = (session.total_details?.amount_tax ?? 0) / divisor;
        ...
        await supabaseAdmin
          ...
          .update({
            ...
            // reflect the tax-inclusive total back into our own records,
            // converted back to USD so "total"/"tax" stay in our base
            // currency regardless of what currency was actually charged.
            tax: taxInChargedCurrency / fxRate,
            total: totalInChargedCurrency / fxRate,
          })
          .eq("id", orderId)
